...
# Check host access, build on first start, and keep it running for Zara.
nix run .# -- doctor rocm
...
The first start builds the ROCm image and downloads the Qwen and Whisper models. Later starts reuse the image and the persistent model cache. Zara should use:
...
```bash
# A local audio file.
nix run .# -- voice add zara ./zara.mp3 \
...
nix run .# -- voice list
nix run .# -- voice info zara
nix run .# -- voice set-text zara "Corrected exact transcript."
nix run .# -- voice rename interview guest
nix run .# -- voice set-default zara
nix run .# -- voice export zara ./zara-reference.wav
